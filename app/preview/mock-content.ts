...
/**
 * Заглушечный контент шаблонов — исходно для `/preview/[template]`
 * (визуальная проверка без реального заказа), но переиспользуется и
 * публичной Shareable Demo (`/demo/[uuid]`, Фаза 10): показ настоящих
 * текстов/фото в демо запрещён (AGENTS.md §7), а этот контент по
 * определению ничей — безопасен для показа кому угодно.
